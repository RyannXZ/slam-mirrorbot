...
def cancel_mirror(update, context):
    ...
    if dl.status() == MirrorStatus.STATUS_ARCHIVING:
        sendMessage("Arsip sedang diproses, Gabisa dibatalin anjay.", context.bot, update)
    elif dl.status() == MirrorStatus.STATUS_EXTRACTING:
        sendMessage("Ekstrak sedang diproses, Gabisa dibatalin anjay.", context.bot, update)
    elif dl.status() == MirrorStatus.STATUS_SPLITTING:
        sendMessage("Split in Progress, Gabisa dibatalin anjay.", context.bot, update)
    else:
...
def cancel_all(update, context):
    ...
    while True:
        ...
        if dl:
            ...
            break
    sendMessage(f'{count} Unduhan(s) Dibatalkan 🦋', context.bot, update)
